...
export function MenuBar( {
  ...
  const renderHelp = (params) => {
    ...
    return (
        <div className="menu-bar__info" >
          ...
          <p>You have {params.maxActionPoints} total action points (AP) to spend.</p>
          <p>It costs 1 AP to place a blocker on an empty tile. You can always undo this for free.</p>
          <p>It costs {params.naturalBlockerRemovalCost} AP to remove a blocker that you didn't place.</p>
  ...
  const renderLogin = () =>
      user && token
          ? <button className='menu-bar__btn generic__btn' onClick={onLogoutClick}>Logout</button>
          : <button className='menu-bar__btn generic__btn' onClick={onLoginClick}>Login</button>;

  const renderSubmit = () =>
      user && token
          ? <button className='menu-bar__btn generic__btn' onClick={toggleSubmit}>Submit</button>
          : null;
      ...
      <div>
        ...
        <div className="menu-bar">
          <div className="menu-bar__section">
            <button id="help-btn" className="menu-bar__btn generic__btn" onClick={toggleHelp} >?</button>
            <Link className="menu-bar__btn generic__btn" to='/leaderboard'>Leaderboard</Link>
          ...
          <div className="menu-bar__section">
            <button onClick={onResetClick} className="menu-bar__btn generic__btn">Reset</button>
            { renderSubmit() }
            { renderLogin() }
          </div>
        </div>
