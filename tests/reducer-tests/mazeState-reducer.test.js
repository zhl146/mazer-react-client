...
import stateReducer, {
  initialState,
} from '../../src/store/reducers/maze-state.reducer'
import {
  createStaticAction,
  createUpdateAction,
...
test('state reducer should be able to update view parameters', assert => {
  ...
  const mazeOrientation =
    testMaze.params.numColumns > testMaze.params.numRows ? LANDSCAPE : PORTRAIT

  const viewHeight = 1080
  const viewWidth = 1920
  const viewOrientation = LANDSCAPE

  const testAction = createUpdateAction(UPDATE_VIEWPARAMS, {
    height: viewHeight,
    width: viewWidth,
  })
  ...
  assert.deepEqual(
    stateReducer(state, testAction).tileSize,
    30,
    'the screen is large enough that the tiles should be the maximum 30px'
  )
  assert.deepEqual(
    stateReducer(state, testAction).rotateMaze,
